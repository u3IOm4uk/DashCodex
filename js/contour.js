...
async function load(buffer,name,kind=C.APP_CONFIG.sourceKinds.USER){const parsed=D.parse(XLSX.read(buffer,{type:'array',cellDates:true,cellText:false}),XLSX);data=parsed;source=C.source(kind,name);sections=D.sections(data);section=sections[0];category=section.categories[0];const bundled=source.kind===C.APP_CONFIG.sourceKinds.BUNDLED,warnings=data.validation?.warnings?.length||0;$('#source-label').textContent=(bundled?'Тестова книга підключена':'Книгу підключено')+(warnings?' · є попередження':'');$('.demo-badge').textContent=bundled?'ТЕСТОВІ ДАНІ':'ІМПОРТОВАНІ ДАНІ';$('#footer-source').textContent='Джерело: '+source.name;setRange(7);await render();if(warnings)notify(`Книгу підключено з попередженнями: ${warnings}`)}
$('#import-button').onclick=()=>$('#file-input').click();$('#file-input').onchange=async e=>{const file=e.target.files[0];if(!file)return;if(file.size>C.APP_CONFIG.maxImportBytes){notify(`Оберіть книгу розміром до ${Math.round(C.APP_CONFIG.maxImportBytes/1024/1024)} МБ.`);return}try{await load(await file.arrayBuffer(),file.name,C.APP_CONFIG.sourceKinds.USER);notify(data.validation?.warnings?.length?'Книгу підключено з попередженнями':'Книгу підключено')}catch(err){notify('Не вдалося прочитати книгу: '+err.message)}finally{e.target.value=''}};
$$('[data-days]').forEach(b=>b.onclick=()=>{if(!data)return;setRange(b.dataset.days);render()});
function updateDates(event){if(!data)return;let f=$('#from').value,t=$('#to').value;if(!f||!t)return;if(f>t){if(event.target.id==='from'){t=f;$('#to').value=t}else{f=t;$('#from').value=f}}const validation=D.validateDateRange(f,t);if(!validation.valid){notify(validation.reason);$('#from').value=from;$('#to').value=to;return}from=f;to=t;mainPlotWindow=null;$$('[data-days]').forEach(b=>b.classList.remove('active'));render()}
