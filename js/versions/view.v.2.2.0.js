            ...
            if (key.startsWith('bind') && key !== 'bindTemplate') {
                ...
                if (value !== undefined && value !== null) {
                    if (attributeName === 'class') {
                        ...
                        // Якщо оригінальні класи ще не збережені в кеш елемента, зберігаємо їх
                        if (element.dataset.originalClasses === undefined) {
                            element.dataset.originalClasses = element.className || '';
                        }

                        // Беремо початкові статичні класи
                        const original = element.dataset.originalClasses;

                        // Об'єднуємо статичні класи та нові динамічні класи
                        // (фільтруємо порожні значення на випадок, якщо value порожній)
                        const combinedClasses = [original, value].filter(Boolean).join(' ');
