...
class Models {
    ...
    async pageDashBoard(filter) {
        ...
        const categoriesViewModel = Data.global.categories.map((catKey, ind) => {
            ...
            if (isCatMulti) {
                ...
                dbKeys.forEach(k => {
                    ...
                    // Загальна сума підкатегорії за весь розширений період графіку
            ...
            chart.xaxis.tickAmount = datesToChart.length - 1; // Застосовуємо математично точний крок
        ...
        // ----------------------------------------------------
        // КРОК 3.2. Генерація двох карток "Загальний тренд" (Мала для 2-го слайда, Велика для 3-го)
        // ----------------------------------------------------
        ...
        if (isMultiActive) {
            ...
            const dailyTotalData = datesToFetch.map(date => {
                let daySum = 0;
                Data.global.units.forEach(unitKey => {
                    daySum += hierarchyMap.get(date)?.get(unitKey)?.get(dbKeysActive) || 0;
                });
                return daySum;
            });
        ...
        // Осі для малого тренду (2-й слайд)
        ...
        // Осі для великого тренду (3-й слайд)
        ...
        chartTotalTrendBig.chart.height = 477; // Робимо симетричним великому радару
        const totalTrendBigId = 'unit-trend-total-big';
        chartReg[totalTrendBigId] = chartTotalTrendBig;

        // Створюємо самі об'єкти карток трендів
        ...
        // Пушимо малу картку тренду в кінець 2-го слайда
        ...
        // Пушимо малу картку Радара в кінець 1-го слайда
        ...
        return {
            ...
            // ПОВНІСТЮ ЗБАЛАНСОВАНА СТРУКТУРА КАРОСЕЛІ
            slideContent: [{
                id: 'slideContent',
                tpl: 'slide-content',
                carousel: [
