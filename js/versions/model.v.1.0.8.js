...
class Models {
    ...
    async pageDashBoard(filter) {
        ...
        // ----------------------------------------------------
        // КРОК 2. Агрегація даних знизу вгору (Тепер суворо з рядка самого угруповання)
        // ----------------------------------------------------
        datesToFetch.forEach(date => {
            ...
            Data.global.units.forEach(unitKey => {
                ...
                // Зміна логіки: сумарне значення за угруповання беремо ТІЛЬКИ з його особистого рядка nameMd
                ...
                Array.from(targetUnitDbKeys.keys()).forEach(dbKey => {
                    targetUnitDbKeys.set(dbKey, row ? getNum(row, dbKey) : 0);
                });
            });
        });
        ...
        const unitsViewModel = Data.global.units.map(unitKey => {
            ...
            // Рахуємо ТЕКСТОВІ ЦИФРИ на картці підрозділу за datesForCards (вже агреговані з індивідуального рядка в Кроці 2)
            ...
            if (config.squad && config.squad.length > 0) {
                ...
                // Для стовпчиків графіка залишаємо прохід по складових squad
        ...
        // ----------------------------------------------------
        // КРОК 4. Побудова масиву КАТЕГОРІЙ (Суму зчитуємо з рядка "ВСЬОГО за СО:")
        // ----------------------------------------------------
        ...
        const categoriesViewModel = Data.global.categories.map((catKey, ind) => {
            ...
            if (isCatMulti) {
                ...
                dbKeys.forEach(k => {
                    // Рахуємо масив по днях прямо з рядка "ВСЬОГО за СО:"
                    const dailyData = datesToFetch.map(date => {
                        const totalRow = rawData.find(row => row && row['Дата'] === date && row['Угруповання'] === 'ВСЬОГО за СО:');
                        return getNum(totalRow, k);
                    });

                    let s = { name: Data.global.categoriesByTitle[k] || k, data: dailyData };
                    if (Data.global.categoriesChartColor[k]) s.color = Data.global.categoriesChartColor[k];
                    chart.series.push(s);

                    // Сума для відображення на картці за обраний період (datesForCards)
                    let subKeyTotal = 0;
                    datesForCards.forEach(date => {
                        const totalRow = rawData.find(row => row && row['Дата'] === date && row['Угруповання'] === 'ВСЬОГО за СО:');
                        subKeyTotal += getNum(totalRow, k);
                    });
                ...
                const dailyData = datesToFetch.map(date => {
                    const totalRow = rawData.find(row => row && row['Дата'] === date && row['Угруповання'] === 'ВСЬОГО за СО:');
                    return getNum(totalRow, dbKeys);
                });

                let s = { name: Data.global.categoriesByTitle[catKey] || catKey, data: dailyData };
                ...
                datesForCards.forEach(date => {
                    const totalRow = rawData.find(row => row && row['Дата'] === date && row['Угруповання'] === 'ВСЬОГО за СО:');
                    totalSum += getNum(totalRow, dbKeys);
                });
        ...
        const unitsTrendsViewModel = Data.global.units.map(unitKey => {
            ...
            if (isMultiActive) {
                ...
                let s = { name: Data.global.categoriesByTitle[activeCategory] || activeCategory, data: dailyData };
                if (Data.global.categoriesChartColor[activeCategory]) s.color = Data.global.categoriesChartColor[activeCategory];
                chart.series.push(s);
            }
        ...
        // ----------------------------------------------------
        // КРОК 3.2. Генерація двох карток "Загальний тренд" (Зчитування з "ВСЬОГО за СО:")
        // ----------------------------------------------------
        let chartTotalTrend = Data.global.getChartArea();
        let chartTotalTrendBig = Data.global.getChartArea();
        chartTotalTrend.series = [];
        chartTotalTrendBig.series = [];

        if (isMultiActive) {
            dbKeysActive.forEach(k => {
                const dailyTotalData = datesToFetch.map(date => {
                    const totalRow = rawData.find(row => row && row['Дата'] === date && row['Угруповання'] === 'ВСЬОГО за СО:');
                    return getNum(totalRow, k);
                });

                let s = { name: Data.global.categoriesByTitle[k] || k, data: dailyTotalData };
                if (Data.global.categoriesChartColor[k]) s.color = Data.global.categoriesChartColor[k];

                chartTotalTrend.series.push(s);
                chartTotalTrendBig.series.push(s);
            });
        } else {
            const dailyTotalData = datesToFetch.map(date => {
                const totalRow = rawData.find(row => row && row['Дата'] === date && row['Угруповання'] === 'ВСЬОГО за СО:');
                return getNum(totalRow, dbKeysActive);
            });

            let s = { name: Data.global.categoriesByTitle[activeCategory] || activeCategory, data: dailyTotalData };
            if (Data.global.categoriesChartColor[activeCategory]) s.color = Data.global.categoriesChartColor[activeCategory];
        ...
        chartTotalTrend.xaxis.categories = datesToChart.map(d => Models.getDateFormat(d, 'apex'));
        chartTotalTrend.xaxis.tickAmount = datesToChart.length - 1;
        chartTotalTrend.chart.height = 215;
        const totalTrendId = 'unit-trend-total';
        chartReg[totalTrendId] = chartTotalTrend;

        chartTotalTrendBig.xaxis.categories = datesToChart.map(d => Models.getDateFormat(d, 'apex'));
        chartTotalTrendBig.xaxis.tickAmount = datesToChart.length - 1;
        chartTotalTrendBig.chart.height = 477;
        const totalTrendBigId = 'unit-trend-total-big';
        chartReg[totalTrendBigId] = chartTotalTrendBig;

        const unitsModelTotalTrend = {
            id: totalTrendId,
        ...
        const unitsModelTotalTrendBig = {
            id: totalTrendBigId,
            unitName: 'Загальний тренд періоду',
            value: totalRadar,
        ...
        unitsTrendsViewModel.push(unitsModelTotalTrend);
        ...
        if (isMultiActive) {
            dbKeysActive.forEach(k => {
                let radarSquadData = [];
                unitsViewModel.forEach(unit => {
                    const val = (unit.valueSet && typeof unit.valueSet === 'object') ? (unit.valueSet[k] || 0) : 0;
                    radarSquadData.push(val);
                });

                let s = { name: Data.global.categoriesByTitle[k] || k, data: radarSquadData };
                if (Data.global.categoriesChartColor[k]) s.color = Data.global.categoriesChartColor[k];
            ...
            let chartRadarSeries = { name: Data.global.categoriesByTitle[activeCategory] || activeCategory, data: [] };
            unitsViewModel.forEach(unit => {
                chartRadarSeries.data.push(
                    typeof unit.valueSet === 'object' ? Object.values(unit.valueSet).reduce((a, b) => a + b, 0) : unit.valueSet
                );
            });

            if (Data.global.categoriesChartColor[activeCategory]) chartRadarSeries.color = Data.global.categoriesChartColor[activeCategory];
        ...
        chartRadarBig.xaxis.labels.style.fontSize = "1.25rem";
        ...
        // ----------------------------------------------------
        // КРОК 6. Опціональна генерація даних для БпС противника (4-й слайд — Розподіл по ОУВ)
        // ----------------------------------------------------
        let droneTrendsViewModel = [];

        if (activeCategory === 'БпС противника' && Data.global.unmSys && Data.global.unmSysConf) {
            droneTrendsViewModel = Data.global.unmSys.map(droneKey => {
                ...
                // Проходимо по кожному військовому угрупованню (ОУВ), які стануть нашими стовпчиками по осі X
                Data.global.units.forEach(unitKey => {
                    const uConfig = Data.global.unitStructure[unitKey];
                    if (!uConfig) return;

                    // Додаємо назву угруповання в осі X графіка
                    chartCategories.push(uConfig.nameMd);

                    // Сумуємо кількість цього типу дрону суворо в зоні цього угруповання за розширений період (datesToFetch)
                    let unitDroneSum = 0;
                    datesForCards.forEach(date => {
                        const dayRows = rawData.filter(row => row && row['Дата'] === date);
                        const row = dayRows.find(r => r['Угруповання'] === uConfig.nameMd);
                        if (row) {
                            unitDroneSum += getNum(row, config.dbKey);
                        }
                    });

                    chartDataValues.push(unitDroneSum);
                });
                console.log('asdasdasd: ', datesForCards);
                // Окремо рахуємо загальну текстову цифру на картці за обрані дати фільтра (datesForCards)
                datesForCards.forEach(date => {
                    const dayRows = rawData.filter(row => row && row['Дата'] === date);
                    Data.global.units.forEach(unitKey => {
                        const uConfig = Data.global.unitStructure[unitKey];
                        ...
                        const row = dayRows.find(r => r['Угруповання'] === uConfig.nameMd);
                        if (row) {
                            totalDroneCount += getNum(row, config.dbKey);
                        }
                    });
                });

                // Будуємо графік Bar (Data.global.getChartBarOne())
                let chart = Data.global.getChartBarOne();
                chart.series = [{
                    name: config.name,
                    data: chartDataValues // масив сум по кожному юніту
                }];
                chart.test = 'this is bar config';

                // Задаємо осі X як назви угруповань замість дат
                chart.xaxis.categories = chartCategories;

                // Кількість міток дорівнює кількості угруповань (сегментів на 1 менше)
                chart.xaxis.tickAmount = chartCategories.length - 1;
                chart.chart.height = 200;

                const droneRenderId = `drone-trend-${droneKey}`;
                chartReg[droneRenderId] = chart;

                // Повертаємо стандартизовану структуру об'єкта для шаблонізатора
                return {
                    id: droneRenderId,
        ...
        // ----------------------------------------------------
        // КРОК 7. Формування фінальної структури каруселі
        // ----------------------------------------------------
        const carouselSlides = [
            // 1-й слайд: Стовпчики угруповань (Bar) + Малий Радар
            { id: 'carusel-content-1', tpl: 'carousel-inner', active: 'active', [unitModelName]: unitsViewModel },
            // 2-й слайд: Тренди угруповань (Area) + Малий Загальний тренд
            { id: 'carusel-content-2', tpl: 'carousel-inner', active: '', [unitModelName]: unitsTrendsViewModel },
            // 3-й слайд: Великий Радар (`477px`) + Великий Загальний тренд (`477px`)
            { id: 'carusel-content-3', tpl: 'carousel-inner', active: 'two-items', [unitModelName]: [unitsModelRadarBig, unitsModelTotalTrendBig] }
        ];

        // Якщо це категорія БпС противника — динамічно пушимо 4-й слайд із параметром grid-6x2
        if (activeCategory === 'БпС противника' && droneTrendsViewModel.length > 0) {
            carouselSlides.push({
                id: 'carusel-content-4',
                tpl: 'carousel-inner',
                active: 'grid-6x2', // Для 4-го слайду параметр active визначено як grid-6x2
                [unitModelName]: droneTrendsViewModel
            });
        }
