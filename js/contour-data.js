...
function aggregate(data,section,cat,from,to,group){const key=aggregateKey(section,cat,from,to,group),cached=cachedAggregate(data,key);if(cached!==undefined)return cached;const days=[];for(let d=from;d<=to&&days.length<P.maxAggregateDays;d=shift(d,1))days.push(d);const index=sheetIndex(data,section.sheet),sourceDates=(data.sheets[section.sheet]?.dates||[...index.byDate.keys()].sort()).filter(d=>d>=from&&d<=to),raw=sourceDates.flatMap(day=>rawRowsForDay(index,cat,day)),rows=sourceDates.flatMap(day=>selectedRowsForDay(index,section,cat,day,group)),selectedByDay=days.map(day=>selectedRowsForDay(index,section,cat,day,group));const series=cat.fields.map((_,i)=>selectedByDay.map(dayRows=>sum(dayRows.map(r=>values(r,cat,section,group)[i]))));const present=days.filter((_,i)=>series.some(s=>s[i]!==null)).length;const totals=cat.fields.map((_,i)=>sum(rows.map(r=>values(r,cat,section,group)[i])));const requestedDays=inclusiveDays(from,to),result={rows,raw,days,series,totals,present,complete:present===days.length,truncated:requestedDays!==null&&requestedDays>days.length};return storeAggregate(data,key,result)}
