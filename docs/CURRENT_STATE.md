...
- На desktop кнопка `Порівняння` розташована праворуч у рядку перемикачів розділів. На mobile нижня панель містить `Огляд / Розділи / Порівняння`; `Порівняння` відкривається як fixed-панель над нижньою навігацією, тому меню лишається видимим і клікабельним.
- Mobile comparison layout одноколонковий: блоки `Показники`, `Підрозділи` і workspace не виходять за ширину viewport; довгі списки мають власний вертикальний scroll замість обрізання всього блоку.
- First-party resource revision: **46**.
