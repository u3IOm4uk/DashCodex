...
- У панелі `Підрозділи` кнопка `Усі` вибирає всі доступні кінцеві (leaf) вузли дерева без ancestor/descendant overlap; `Скинути` очищає unit scope. Окрема кнопка `Загальний підсумок` більше не відображається.
...
- First-party resource revision: **52**.
