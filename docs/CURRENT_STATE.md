...
- Перемикач типу графіка в конструкторі повторно використовує стандартний `segmented`-патерн інших графіків і має лише icon-only кнопки `trend / bars`; перемикач `Нормалізовано / Абсолютні` використовує той самий компактний segmented-стиль із текстовими назвами режимів.
