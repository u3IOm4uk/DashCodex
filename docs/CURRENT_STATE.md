...
- Для нормалізації перше доступне ненульове значення серії = 100; `null` не перетворюється на 0.
...
- Таблиця порівняння повторно використовує чинний патерн `panel details-panel + table-wrap`, тому типографіка, відступи, sticky header, row hover і scroll-поведінка збігаються з таблицями «Деталей».
- First-party resource revision: **47**.
